...
for i = first_im_number:last_im_number
    file_name = [dir_name '/' 'img_' sprintf('%05d',i) '.' extension];
    if ~exist(file_name, 'file')
        ...
        [detected_gate(p,:),gates_candidate_corners{p}, cf{p}] = run_detection_corner_refine_img(dir_name, i, graphics, ROTATE_90, rotation, extension);
        if graphics
            minimal_fitness_for_plotting = 0.7;
            h = figure();
            RGB = imread([dir_name '/' 'img_' sprintf('%05d',i) '.' extension]);
            RGB = double(RGB) ./ 255;
            if(ROTATE_90)
                RGB = imrotate(RGB, 90);
            end
