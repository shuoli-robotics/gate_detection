function [gate_corners_x,gate_corners_y, x, y, s, best_fit, valid, height_left, height_right, angle, arm_angle_1, arm_angle_2, s_left, s_right] ...
            = fit_window_to_points_free_lc(points, x0, y0, size0, weights,Response)
gate_corners = 0;
...
n_generations = 2;%was 30
best_fit = 0;%1000000000;
...
for g = 1:n_generations
    ...
    for i = 1:n_individuals
        ...
        fits(i) = mean_distance_to_polygon_line_check(Population(i, :), params,Response);
    end
    [v, ind] = max(fits);%min(fits)
    if(v > best_fit)
    ...
    if(g < n_generations)
        ...
        Mutation = 10 * rand(n_individuals, genome_length) - 5;%2.5;
